---
title: "Parrotfish_BranchMPAmodel"
...
MPA_parrotfish <- MPA.model(r=0.566, K=1.22, u.out=0.2485, ncells=100,
          MPA.width=5, nsteps=5, mrate=0.5)

NoMPA_parrotfish <- MPA.model(r=0.566, K=1.22, u.out=0.2485, ncells=100,
          MPA.width=0, nsteps=5, mrate=0.5) #assuming no MPA MPA.width set to 0 everything else remains the same 
```


```{r}
MPA_parrotfish.df<- data.frame(MPA_parrotfish[-1,])%>%
  mutate(year=c(2015:2019))%>%
  rename(c("leave"="MPAleave", "arrive"="MPAarrive", "sur"="MPAsurplus", "catch"= "MPAcatch", "biomass"="MPAbiomass"))
NoMPA_parrotfish.df<-data.frame(NoMPA_parrotfish[-1,])%>%
    mutate(year=c(2015:2019))

merge<-merge(MPA_parrotfish.df, NoMPA_parrotfish.df, by="year")
