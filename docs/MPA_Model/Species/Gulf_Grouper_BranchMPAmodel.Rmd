---
title: "GulfGrouper_BranchMPAmodel"
...
MPA_gulfgrouper <- MPA.model(r=0.062, K=922.46, u.out=0.2485, ncells=100,
          MPA.width=5, nsteps=5, mrate=0.5)

NoMPA_gulfgrouper <- MPA.model(r=0.062, K=922.46, u.out=0.2485, ncells=100,
...
```{r}
MPA_gulfgrouper.df<- data.frame(MPA_gulfgrouper[-1,])%>%
  mutate(year=c(2015:2019))%>%
  rename(c("leave"="MPAleave", "arrive"="MPAarrive", "sur"="MPAsurplus", "catch"= "MPAcatch", "biomass"="MPAbiomass"))
NoMPA_gulfgrouper.df<-data.frame(NoMPA_gulfgrouper[-1,])%>%
    mutate(year=c(2015:2019))

merge<-merge(MPA_gulfgrouper.df, NoMPA_gulfgrouper.df, by="year")
