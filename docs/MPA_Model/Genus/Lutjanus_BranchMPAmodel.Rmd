...
MPA_lutjanus <- MPA.model(r=0.2822, 
                 K=3058.21000,
                 B0=980.9472, 
                 fishing=0.1, 
                 patches=100,
...
NoMPA_lutjanus <- MPA.model(r=0.2822,
                   K=3058.21000, 
                   B0=980.9472, 
                   fishing=0.1, 
                   patches=100,
...
MPA_lutjanus.df <- data.frame(MPA_lutjanus[-1,])%>%
  mutate(year=c(2015:2064), State="MPA")

NoMPA_lutjanus.df <- data.frame(NoMPA_lutjanus[-1,])%>%
    mutate(year=c(2015:2064), State="No_MPA")

merge_lutjanus <- rbind(MPA_lutjanus.df, NoMPA_lutjanus.df)
...
ggplot(data = merge_lutjanus, aes(x = year, y = biomass, colour = State))+
  geom_point()+
  geom_line()+
  labs(x="Year", y="Projected biomass (MT)")+
  theme_classic()+
...
```{r}
ggplot(data = merge_lutjanus, aes( x = year, y = catch, colour = State))+
  geom_point()+
  geom_line()+
